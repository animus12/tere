				...
				<form id="manage-receiving">
					...
					<div class="row">
						<div class="form-group col-sm-3">
							...
							<select name="item_id" id="item_id" class="product-class custom-select custom-select-sm select2">
								<option></option>
								<?php
								$items = $conn->query("SELECT * FROM items order by name asc");
								while($row = $items->fetch_assoc()):
								?>
								<option value="<?php echo $row['id'] ?>" data-prod="<?php echo $row['price']; ?>" data-code = "<?php echo $row['item_code'] ?>"><?php echo ucwords($row['name']) ?></option>
							<?php endwhile; ?>
							</select>
						</div>
						<div class="form-group col-sm-3">
						...
						<div class="form-group col-sm-3">
							...
							<input type="text" class="form-control form-control-sm number text-right" readonly id="cost">
						</div>
